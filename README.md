# M-OBS Wiki

**Mantle Observability Stack - Complete Documentation**

Welcome to the M-OBS Wiki! This comprehensive documentation covers everything you need to know about the Mantle Observability Stack.

## 📚 Documentation Structure

### [📖 Technical Documentation](/technical)
Complete technical reference for developers and operators.

- [Architecture Overview](/technical/architecture.md)
- [Setup Guide](/technical/setup.md)
- [API Reference](/technical/api-reference.md)
- [Database Schema](/technical/database-schema.md)
- [Deployment Guide](/technical/deployment.md)
- [Monitoring & Maintenance](/technical/monitoring.md)

### [💼 Business Documentation](/business)
Business case, market analysis, and growth strategy.

- [Value Proposition](/business/value-proposition.md)
- [Market Analysis](/business/market-analysis.md)
- [Use Cases](/business/use-cases.md)
- [Roadmap](/business/roadmap.md)
- [Competitive Analysis](/business/competitive-analysis.md)

### [❓ FAQ & Pitching](/faq)
Frequently asked questions and pitch deck preparation.

- [General FAQ](/faq/general.md)
- [Technical FAQ](/faq/technical.md)
- [Business FAQ](/faq/business.md)
- [Pitching Guide](/faq/pitching.md)
- [Demo Scenarios](/faq/demo-scenarios.md)

---

## 🚀 Quick Links

- **🌐 Production URL:** https://m-obs.hackon.team/ ⭐ **[TRY IT NOW]**
- **Live Demo (Vercel):** https://m-obs-project.vercel.app/
- **API Docs:** https://m-obs-api.onrender.com/docs
- **GitHub Repositories:**
  - Main: https://github.com/hackonteam/m-obs
  - Backend: https://github.com/hackonteam/m-obs-backend
  - Frontend: https://github.com/hackonteam/m-obs-frontend
  - Database: https://github.com/hackonteam/m-obs-database

---

## 📊 Project Overview

**M-OBS (Mantle Observability Stack)** is a comprehensive real-time monitoring and analytics platform for the Mantle blockchain network.

### Key Features
- ✅ Real-time transaction monitoring
- ✅ RPC provider health tracking
- ✅ Gas price analysis
- ✅ Error detection & decoding
- ✅ Custom alerting system
- ✅ Beautiful dashboard UI

### Technology Stack
- **Frontend:** SvelteKit + TailwindCSS + DaisyUI
- **Backend:** Python + FastAPI
- **Worker:** Async Python pipelines
- **Database:** PostgreSQL (Supabase)
- **Deployment:** Vercel + Render.com

---

## 👥 Team

**HackOn Team Vietnam** in collaboration with **Phu Nhuan Builder**

---

## 📄 License

MIT License - Copyright (c) 2026 HackOn Team Vietnam and Phu Nhuan Builder

---

## 🔄 Last Updated

January 15, 2026
